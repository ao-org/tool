...
Dim i As Long
    ...
    FileNum = FreeFile
    Open DataPath & "graficos.ini" For Input As #FileNum
...
Dim s() As String
    ...
    'Load the Grhs from the texture that are already in graficos.ini
    LoadOldGrhs
...
Dim i As Long
    ...
    Else
        If frmMain.RowsTxt.Text <> -1 Then
            i = frmMain.RowsTxt.Text
            Else
    ...
    Else
    If frmMain.ColumnsTxt.Text <> -1 Then
        i = frmMain.ColumnsTxt.Text
    Else
...
End Sub
'Modificado por ReyarB 31/01/2021
...
Dim GrhLine As Long
    ...
    For y = Val(frmMain.StartYTxt.Text) To TexHeight Step GridHeight
        For x = Val(frmMain.StartXTxt.Text) To TexWidth Step GridWidth
            ...
            If x >= 0 Then ' And x < Val(frmMain.ColumnsTxt.Text * frmMain.GridWidthTxt.Text) Then
                If y >= 0 Then 'And y < Val(frmMain.RowsTxt.Text * frmMain.GridHeightTxt.Text) Then
                    If x < Val(frmMain.ColumnsTxt.Text * frmMain.GridWidthTxt.Text + frmMain.StartXTxt.Text) Then 'TexWidth Then
                        If y < Val(frmMain.RowsTxt.Text * frmMain.GridHeightTxt.Text + frmMain.StartYTxt.Text) Then 'TexHeight Then
...
Dim c As Long
    ...
    IsFreeGrh = (LenB(Var_Get(Data2Path & "graficos.ini", "A", "Grh" & GrhIndex)) = 0)
